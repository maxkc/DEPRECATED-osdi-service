...
function listScripts(req, res) {
  ...
  var url = 'https://api.securevan.com/services/v3/EventRoleService.asmx?WSDL';
  var args={};
  //var args = {
  //  'criteria': {
  //    'Status': 'Active',
  ...
  soap.createClient(url, function (err, client) {
    ...
    client["ListEvents"](args, function (err, result) {

      if (err) {
        res.send(result.body);
      } else {
        res.send(result.ListEventsResult);
...
module.exports = function (app) {
  app.get('/api/v1/soapevents', listScripts);
  app.get('/api/v1/soapevents/:id', getScript);
