...
/**
 * Creates a bridge between OSDI and VAN using the osdi and ngpvan-api-client modules.
 */
 ...
 */
function sendSingleResourceResponse(resourcePromise, translator, resourceType, res) {
  resourcePromise.
    then(function(vanResource) {
  	  var osdiResource = translator(vanResource);
      return res.status(200).send(osdiResource);
    }).
    catch(client.errors.Forbidden, 
      function(ex) { return osdi.response.unauthorized(res); }).
    catch(client.errors.NotFound, 
      function(ex) { return osdi.response.badRequest(res, resourceType)(ex); }).
    catch(client.errors.Unexpected, 
      function (ex) { return osdi.response.badRequest(res, resourceType)(ex); });
}

/**
 * Send an OSDI "single-resource" response, given the VAN resource promise and 
 * OSDI pagination parameters, and a function for translating a single VAN resource 
 * into a single OSDI resource.
 */
function sendMultiResourceResponse(resourcePromise, osdiPaginationParams, translator,
 resourceType, res) {
  resourcePromise.
    then(function(vanResources) {
      var totalRecords = vanResources.count;
      
      var totalPages = Math.ceil(totalRecords / osdiPaginationParams.perPage);
      var answer = osdi.response.createPaginatedItem(osdiPaginationParams.page,
        osdiPaginationParams.perPage, totalPages, totalRecords, resourceType);
